    ...
    int minRestartTPM) // random restart budget for outer iterations >= 2
{
  ...
  {
    warning("Warning: Algorithm terminated without finding a feasible solution (within %.3e tolerance). Last iterate has a feasibility violation of %.3e. You may want to rerun the algorithm with more iterations (maxIter)", feasibilityTolerance, violation_best);
  }

    ...
    int minRestartTPM) // random restart budget for outer iterations >= 2
{
  ...
  {
    warning("Warning: Algorithm terminated without finding a feasible solution (within %.3e tolerance). Last iterate is %.3e feasible. You may want to rerun the algorithm with more iterations (maxIter)", feasibilityTolerance, violation_best);
  }

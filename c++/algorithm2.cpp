/*
to compile:
1. go to the folder "c++"
2. bash .\script.sh
   2.1 change the first line in .\script.sh:
       libeigen="change the content of this to the absolute path of your eigen-3.4.0"
   2.2 the website of Eigen:
...
{
    return (solution.transpose() * prob.Sigma * solution)[0];
}
...
vector<int> selectperm2(VectorXd x, int k)
{
    ...
    partial_sort(
        z.begin(), z.begin() + k, z.end(), [&](int i, int j) -> bool
    ...
    double violation_tolerance = 1e-4)
{
    ...
    for (int theIter = 1; theIter <= numIters; theIter++)
    {
        ...
        stepSize = (theIter < fastPeriod ? 0.01 : 0.05) * (theIter < slowPeriod ? violation : ofv_overall / violation); //these magic numbers are extremely ugly
        if (verbose)
        {
...
extern "C"{
        ...
        double violation_tolerance = 1e-4)
    {
        ...
        int* ks = (int*) malloc(r * sizeof(int));
        for (int i = 0; i < r; i++) {
            ks[i] = (int)ks_double[i];
        }

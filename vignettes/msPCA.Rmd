...
components. The header names the constraint definition being reported and
confirms that it is the one used to fit.

```{r summary}
summary(res)
```

Both methods take an optional `C` argument, but it is no longer needed:
`mspca()` stores the variance figures, the loading names, and the pairwise
violations in the fitted object. `C` is retained only for objects created by
msPCA 0.5.0 or earlier.
...
- `scale` (default `TRUE` in `mspca()`, `FALSE` in `tpm()`) divides by column
...
correlation-matrix result above. As with `type = "Sigma"`, everything
`print()` and `summary()` report is stored in the fitted object, so no
matrix has to be passed back in — which matters here, since the point of
`type = "X"` is never to form the $p \times p$ matrix in the first place.
...
`summary()` reports the violations under the constraint that was actually
enforced: the fitted `feasibilityConstraintType` is stored in the object and
reused automatically, and the printed output names the definition in use.
There is no need to repeat the setting, and no risk of reading an
orthogonality summary for a solution fitted under zero correlation.

Both sets of scores are computed at fit time and stored, so the solution can
be inspected under the other definition without a refit:
...
$\mathrm{tr}(C)/p$, so it is invariant to a rescaling of `C` and comparable
between a covariance matrix and the corresponding correlation matrix. The
orthogonality violation needs no such normalization. Note that
`res$feasibility_violation`, the quantity the solver compares against
`feasibilityTolerance`, additionally includes the diagonal norm terms and so
is a slightly larger number than the off-diagonal diagnostic above.

...
($k_t = p$). In the zero-correlation case the implementation uses $\boldsymbol{\Sigma}$ divided by
the average variance $\mathrm{tr}(\boldsymbol{\Sigma})/p$. This leaves the feasible set unchanged and
makes both the penalty scale and the violation measure below invariant to a rescaling of the
data.
...
both computed by `feasibility_violation_off()`. The second is normalized by the average
...
$\boldsymbol{C}$ and the unnormalized sum would depend on the units of the data. With the
normalization, a fixed `feasibilityTolerance` means the same thing for a covariance matrix and
...
final convergence to feasibility. Under the orthogonality constraint the penalties are
weighted per component by the variance that component explains at the first iteration, so that
all penalty terms are on a comparable scale regardless of the magnitude of
$\boldsymbol{\Sigma}$; under the zero-correlation constraint the weights are 1, the scaling of
$\boldsymbol{C}$ having already done that. See @cory2022sparse
